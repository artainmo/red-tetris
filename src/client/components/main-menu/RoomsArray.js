				...
				<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
					<div style={{alignItems: "center", width: "100%"}}>
						<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
							...
							<SmallButton textContent={loading ? "Refreshing..." : "Refresh"} onClick={fetchRooms} />
						</div>
						<div style={delimiterStyle}></div>
					</div>
				</div>

				<div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", width: "100%", height: "60%", overflowY: "auto", marginTop: 10 }}>
					{rooms.length === 0 ? (
						<p>No rooms to join right now</p>
					) : (
						rooms.map((r, index) => (
							<div style={{ display: "flex", justifyContent: "space-between", width: "100%" }} key={r.id || index}>
								<p style={{ margin: "auto", marginLeft: "0" }}>
									#{index} {r.host} room ({r.players.length} players)
								</p>
								<SmallButton textContent={"Join"} onClick={() => joinMultiplayer(r.id)} />
							</div>
						))
					)}
				</div>
